...
const SupplierAccountStatementTab = ({ supplier }) => {
    ...
    const statementData = useMemo(() => {
        ...
        const supplierPurchases = allPurchases.filter(p => p.proveedor_id === supplier.id);
        
        const totalPurchased = supplierPurchases.reduce((acc, p) => acc + (Number(p.monto) || 0), 0);
        
        const totalPaid = supplierPurchases.reduce((acc, p) => acc + (Number(p.pagado) || 0), 0);
    ...
    const handleExportPDF = () => {
        ...
        statementData.purchases.forEach(item => {
            ...
            tableRows.push([
                item.facturaRef || item.id,
                format(new Date(item.fecha_emision || item.fecha_vencimiento), 'dd/MM/yyyy'),
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {statementData.purchases.length > 0 ? statementData.purchases.map(p => {
                                ...
                                return (
                                <TableRow key={p.id}>
                                    <TableCell>#{p.facturaRef || p.id}</TableCell>
                                    <TableCell>{format(new Date(p.fecha_emision || p.fecha_vencimiento), 'dd/MM/yyyy')}</TableCell>
                                    <TableCell>${monto.toFixed(2)}</TableCell>
                                    <TableCell className="font-semibold">${saldoPendiente.toFixed(2)}</TableCell>
